...
├── components/                  (sub-components - not exposed as routes)  
│   ├── ClientSummary.tsx  
│   ├── CarsLoadingView.tsx  
│   ├── CarsErrorView.tsx  
...
├── GenerateBookingFlow  
     ├── BookingForm     │    ├── CarSelect     │    ├── BookingDatePicker     │    ├── DaysField     │    └── SubmitButton     ├── BookingSuccessNotification  (imperative via exported functions)     ├── BookingErrorNotification    (imperative via exported functions)     └── BookingResultDisplay```  
```
...
import {   
  GenerateBooking,   
  CarSelect,   
showBookingSuccess } from './generateBooking';  
```
...
The sub-components are placed in the `components/` subdirectory to prevent Vaadin's file-based router from automatically creating routes for them. Only `GenerateBooking.tsx` should be exposed as a route since the other components require props and are not meant to be standalone pages.  
